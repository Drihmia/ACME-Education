...
export const StudentProfile = ({ user, openModal }: { user: User, openModal: (item: any) => void }) => {
      ...
      <div className="w-full h-full grid md:grid-cols-[1fr_3fr] gap-4 md:gap-8">
        ...
        <div className="w-full h-full flex flex-col justify-between p-4 md:p-8">
          ...
          <ul className="w-full flex flex-col gap-1">
            ...
            <li className="text-lg md:text-xl font-medium">
              Teacher&apos;s email:{" "}
              <span className="font-normal">{profile.teacher_email === "Null" ? "N/A" : profile.teacher_email}</span>
            </li>
            <li className="text-lg md:text-xl font-medium">
              Class ID:{" "}
              <span className="font-normal">{profile.class_id}</span>
            </li>
          </ul>
          </div>
